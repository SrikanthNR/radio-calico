// Cache now-playing rating DOM references once
const _npUp       = document.getElementById('np-up');
const _npDown     = document.getElementById('np-down');
const _npUpCount  = document.getElementById('np-up-count');
const _npDownCount = document.getElementById('np-down-count');
...
async function rateNp(value) {
  ...
  if (!res.ok) return;
  applyRatingUI(_npUp, _npDown, _npUpCount, _npDownCount, await res.json());
}
